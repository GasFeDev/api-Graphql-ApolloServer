...
import { createSetContractURI } from '../txLogger/method';
...
const allMethods = ['setContractURI'];

export const setContractURICall = async (
  args: any[],
  tokenAddress: string,
  method: string,
  _uri: string,
  ...
  try {
    ...
    if (result) {
      ...
      await createSetContractURI(updateLoggerPayload);
